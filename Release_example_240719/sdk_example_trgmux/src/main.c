...
void
trgmux_lpuart_test(void)
{
    uint8_t  idx        = 0;
    g_pte4              = 0;
    g_trgmux_on         = false;

    print((const char *)"trgmux_lpuart0 test code: switch function by pte4(touch)\n");
    /* Enable button port IRQ */
    int_enable_irq(PORT_IRQn);
    LPUART0->PINCFG = 0x03; //trgmux_lpuart0 out: LPUART0_TRG TX
    ...
    for (;;)
    {
        ...
        if (g_pte4 == 1)
        {
            if (g_trgmux_on == false)
            {
                g_trgmux_on = true;
                TRGMUX->TRGMUXn[TRGMUX_LPUART0_INDEX] =
                    (TRGMUX->TRGMUXn[TRGMUX_LPUART0_INDEX] & ~TRGMUX_TRGMUXn_SEL0_MASK) |
                    TRGMUX_TRGMUXn_SEL0(TRGMUX_TRIG_SOURCE_VDD); // Sel0(out0) = HIGH
                print((const char *)"trgmux_lpuart0 trigger : enable \n");
            }
            else
            {
                g_trgmux_on = false;
                print((const char *)"\ntrgmux_lpuart0 trigger : disable \n");
                TRGMUX->TRGMUXn[TRGMUX_LPUART0_INDEX] =
                    (TRGMUX->TRGMUXn[TRGMUX_LPUART0_INDEX] & ~TRGMUX_TRGMUXn_SEL0_MASK) |
                    TRGMUX_TRGMUXn_SEL0(TRGMUX_TRIG_SOURCE_DISABLED); // Sel0(out0) = LOW
            }
            g_pte4 = 0;
        }
        print((const char *)"o");
