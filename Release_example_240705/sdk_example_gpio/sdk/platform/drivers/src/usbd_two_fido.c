...
#define USBD_HID_CPL_FIDO_IF0_ALT       (0u)
...
typedef struct _usbd_dev_if_state_t
{
    ...
    uint8_t                     fido_if0_report_id0_idle_rate;
    uint8_t                     fido_if1_report_id0_idle_rate;
...
static const usbd_fs_comp_cfg_desc_t s_usbd_fs_comp_cfg_desc = {
    ...
    .usb_if0_desc = {.blength       = sizeof(usb_std_if_desc_t),
                     ...
                     .balt_setting  = USBD_HID_CPL_FIDO_IF0_ALT,
...
usbd_init(void)
{
    ...
    p_cfg_desc                         = &(s_usbd_fs_comp_cfg_desc.usb_cfg_desc);
    ...
    p_state_ptr->fido_if0_report_id0_idle_rate =
        0u; /* with a 4 millisecond resolution. 0 is indefinite */
    p_state_ptr->fido_if1_report_id0_idle_rate =
        0u; /* with a 4 millisecond resolution. 0 is indefinite */
    ...
    fsusb_set_intrpt_enable_bit(
        p_base, FSUSB_INT_EP6RX_BM | FSUSB_INT_EP5TX_BM | FSUSB_INT_EP4RX_BM | FSUSB_INT_EP3TX_BM);
...
usbd_pipe_sram3_acquire_job(uint8_t group_ep_num, uint8_t *p_income, uint32_t byte_cnt)
{
    ...
    usbd_pipe_sram3_queue_job(USBD_EP_NB3, p_income, byte_cnt);
}
...
usbd_ctrl_std_cmd_proc(void)
{
    ...
    switch (p_dev_req->brequest)
    {
        case USB_REQ_GET_STATUS:
        {
            switch (p_dev_req->bmreq_type)
            {
                ...
                case (USB_RT_DIR_DEV2HOST | USB_RT_TYPE_STANDARD | USB_RT_RECIPIENT_ENDPOINT):
                {
                    ...
                    else if (USBD_EP3_DI == p_dev_req->windex.sb_type.lbyte)
                    {
                        p_state_ptr->p_ep0_buffer[0u] = fsusb_get_ep3_stall_st(p_base);
                    }
                    else if (USBD_EP4_DO == p_dev_req->windex.sb_type.lbyte)
                    {
                        p_state_ptr->p_ep0_buffer[0u] = fsusb_get_ep4_stall_st(p_base);
                    }
                    else if (USBD_EP5_DI == p_dev_req->windex.sb_type.lbyte)
                    {
                        p_state_ptr->p_ep0_buffer[0u] = fsusb_get_ep5_stall_st(p_base);
                    }
                    else if (USBD_EP6_DO == p_dev_req->windex.sb_type.lbyte)
                    {
                        p_state_ptr->p_ep0_buffer[0u] = fsusb_get_ep6_stall_st(p_base);
                    }
                    else
                    {
                        usbd_ctrl_ep0_set_stall(); /**< Return stall for command error. */
        ...
        case USB_REQ_CLEAR_FEATURE:
        {
            ...
            {
                if (USBD_EP3_DI == p_dev_req->windex.sb_type.lbyte)
                {
                    usbd_intrpt_in_ep3_set_stall(false);
                }
                else if (USBD_EP4_DO == p_dev_req->windex.sb_type.lbyte)
                {
                ...
                {
                    usbd_intrpt_in_ep5_set_stall(false);
                }
                else if (USBD_EP6_DO == p_dev_req->windex.sb_type.lbyte)
                {
                    usbd_intrpt_out_ep6_set_stall(false);
                }
                else
                {
                    usbd_ctrl_ep0_set_stall(); /**< Return stall for command error. */
        ...
        case USB_REQ_SET_FEATURE:
        {
                     ...
                     (USB_REQ_FEAT_ENDPOINT_HALT == p_dev_req->wvalue.sb_type.lbyte))
            {
                if (USBD_EP3_DI == p_dev_req->windex.sb_type.lbyte)
                {
                    usbd_intrpt_in_ep3_set_stall(true);
                }
                else if (USBD_EP4_DO == p_dev_req->windex.sb_type.lbyte)
                {
                ...
                {
                    usbd_intrpt_in_ep5_set_stall(true);
                }
                else if (USBD_EP6_DO == p_dev_req->windex.sb_type.lbyte)
                {
                    usbd_intrpt_out_ep6_set_stall(true);
                }
                else
                {
                    usbd_ctrl_ep0_set_stall(); /**< Return stall for command error. */
        ...
        case USB_REQ_GET_DESCRIPTOR:
        {
            if (USB_RT_RECIPIENT_DEVICE == (p_dev_req->bmreq_type & USB_RT_RECIPIENT_MASK))
            {
                switch (p_dev_req->wvalue.sb_type.hbyte)
                {
                    ...
                    {
                        p_desc   = (const uint8_t *)&s_usbd_fs_comp_cfg_desc;
                        byte_cnt = sizeof(s_usbd_fs_comp_cfg_desc);
                    }
                    break;
                    case USB_DESC_STRING: // 03h
                    {
                        switch (p_dev_req->wvalue.sb_type.lbyte)
                        {
                            ...
                            break;
                            case USB_STR_IDX004: /**< String Index4 */
                                p_desc   = (const uint8_t *)&s_usbd_str_sn2;
                                byte_cnt = sizeof(s_usbd_str_sn2);
                                break;
                            default:
                            {
                                usbd_ctrl_ep0_set_stall(); /**< Return stall for command error. */
            ...
            else if (USB_RT_RECIPIENT_INTERFACE == (p_dev_req->bmreq_type & USB_RT_RECIPIENT_MASK))
            {
                switch (p_dev_req->wvalue.sb_type.hbyte)
                {
                    ...
                    {
                        if (p_dev_req->windex.sb_type.lbyte ==
                            s_usbd_fs_comp_cfg_desc.usb_if0_desc.bif_number)
                        {
                            p_desc   = (const uint8_t *)&(s_usbd_fs_comp_cfg_desc.fido0_desc);
                            byte_cnt = sizeof(s_usbd_fs_comp_cfg_desc.fido0_desc);
                        }
                        else if (p_dev_req->windex.sb_type.lbyte ==
                                 s_usbd_fs_comp_cfg_desc.usb_if1_desc.bif_number)
                        {
                            p_desc   = (const uint8_t *)&(s_usbd_fs_comp_cfg_desc.fido1_desc);
                            byte_cnt = sizeof(s_usbd_fs_comp_cfg_desc.fido1_desc);
                        }
                        else
                        {
                            usbd_ctrl_ep0_set_stall(); /**< Return stall for command error. */
                    ...
                    {
                        p_desc   = (const uint8_t *)s_fido_report.report;
                        byte_cnt = sizeof(s_fido_report);
                    }
                    break;
                    default:
                    {
...
usbd_ctrl_class_cmd_proc(void)
{
    ...
    switch (p_dev_req->brequest)
    {
        ...
        {
            if (p_dev_req->windex.sb_type.lbyte == s_usbd_fs_comp_cfg_desc.usb_if0_desc.bif_number)
            {
                p_state_ptr->p_ep0_buffer[0u] = p_state_ptr->fido_if0_report_id0_idle_rate;
            }
            else if (p_dev_req->windex.sb_type.lbyte ==
                     s_usbd_fs_comp_cfg_desc.usb_if1_desc.bif_number)
            {
                p_state_ptr->p_ep0_buffer[0u] = p_state_ptr->fido_if1_report_id0_idle_rate;
            }
            else
            {
                usbd_ctrl_ep0_set_stall(); /**< Return stall for command error. */
        ...
        case USBD_REQ_SET_IDLE:
        {
            if (p_dev_req->windex.sb_type.lbyte == s_usbd_fs_comp_cfg_desc.usb_if0_desc.bif_number)
            {
                p_state_ptr->fido_if0_report_id0_idle_rate = p_dev_req->wvalue.sb_type.hbyte;
                if ((USBD_SC_BOOT == s_usbd_fs_comp_cfg_desc.usb_if0_desc.bif_sub_class) &&
                    (0u == p_dev_req->wvalue.sb_type.hbyte))
                {
                    ...
                    return;
                     ...
                     s_usbd_fs_comp_cfg_desc.usb_if1_desc.bif_number)
            {
                p_state_ptr->fido_if1_report_id0_idle_rate = p_dev_req->wvalue.sb_type.hbyte;
                if ((USBD_SC_BOOT == s_usbd_fs_comp_cfg_desc.usb_if1_desc.bif_sub_class) &&
                    (0u == p_dev_req->wvalue.sb_type.hbyte))
                {
                    usbd_ctrl_ep0_set_stall(); /**< Return stall for command error. */
                    return;
