...
sim_set_lpo_clks(SIM_t   *p_base,
                 bool     b_enable_lpo1k,
                 bool     b_enable_lpo10k,
                 uint32_t source_lpo_clk,
                 uint32_t source_rtc_clk)
{
    uint32_t reg_val = p_base->LPOCLKS;

    reg_val &= ~(SIM_LPOCLKS_LPO1KEN_MASK | SIM_LPOCLKS_LPO10KEN_MASK | SIM_LPOCLKS_LPOCLKSEL_MASK |
                 SIM_LPOCLKS_RTCCLKSEL_MASK);

    reg_val |= SIM_LPOCLKS_LPO1KEN(b_enable_lpo1k ? 1UL : 0UL);
    reg_val |= SIM_LPOCLKS_LPO10KEN(b_enable_lpo10k ? 1UL : 0UL);
...
/*FUNCTION*********************************************************************
 * Function Name : sim_get_lpo10k_status
 * Description   : Get SIM LPO 10KHz clock gating status (LPO_10K_CLOCK).
 *END*************************************************************************/
static inline bool
sim_get_lpo10k_status(const SIM_t *p_base)
{
    return (((p_base->LPOCLKS & SIM_LPOCLKS_LPO10KEN_MASK) >> SIM_LPOCLKS_LPO10KEN_SHIFT) != 0U)
               ? true
               : false;
}
...
{
    return (((p_base->LPOCLKS & SIM_LPOCLKS_LPO1KEN_MASK) >> SIM_LPOCLKS_LPO1KEN_SHIFT) != 0U)
               ? true
               : false;
}
